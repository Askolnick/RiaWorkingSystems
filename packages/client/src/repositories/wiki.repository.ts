...
- Pricing sheets: ./resources/pricing`,
  ...
  private loadMockData() {
    ...
    if (!this.getStorage().length) {
      this.setStorage(this.mockSpaces as any);
    }
  }
  ...
  async createSpace(data: CreateWikiSpaceData): Promise<WikiSpace> {
    ...
    const spaces = this.getStorage();
    spaces.push(space);
    this.setStorage(spaces);
    return space;
  }
  ...
  async getSpaceWithPages(id: string): Promise<WikiSpaceWithPages> {
    const space = this.getStorage().find(s => s.id === id);
  ...
  async getPage(spaceId: string, pageId: string): Promise<WikiPageWithRelations> {
    ...
    const space = this.getStorage().find(s => s.id === spaceId)!;
  ...
  async searchWiki(options: WikiSearchOptions): Promise<WikiSearchResult[]> {
    ...
    // Search spaces
    const spaces = this.getStorage().filter(space => {
    ...
    pages.forEach(page => {
      const space = this.getStorage().find(s => s.id === page.spaceId);
      results.push({
        type: 'page',
        id: page.id,
        title: page.title,
...
export const wikiRepository = new MockWikiRepository();
